...
function Hero() {
  ...
  return (
    <section style={{ position: "relative", height: "115vh", overflow: "hidden", background: "#14161C" }}>
      ...
      <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", justifyContent: "center", padding: "0 clamp(20px,5vw,80px)", paddingTop: 80 }}>
        <div style={{ maxWidth: 900 }}>
          ...
          <div style={{ display: "flex", gap: 16, flexWrap: "wrap", opacity: v ? 1 : 0, transform: v ? "none" : "translateY(12px)", transition: "all .7s cubic-bezier(.16,1,.3,1) .75s" }}>
            ...
            <Link to="/work" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(255,255,255,.12)", color: "#FAF8F5", padding: "15px 32px", borderRadius: 50, textDecoration: "none", fontSize: 13, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", transition: "all .25s", border: "1px solid rgba(255,255,255,.25)", backdropFilter: "blur(8px)" }}
...
function ServicesPreview() {
  ...
  return (
    <section style={{ background: "#FAF8F5", padding: "110px clamp(20px,5vw,80px)" }}>
      <div style={{ maxWidth: 1320, margin: "0 auto" }}>
        <div ref={ref} style={{ textAlign: "center", marginBottom: 64 }}>
          <h2 style={{ fontSize: "clamp(30px,3vw,50px)", fontWeight: 800, color: "#14161C", letterSpacing: "-1px", opacity: v ? 1 : 0, transform: v ? "none" : "translateY(20px)", transition: "all .7s cubic-bezier(.16,1,.3,1) .1s" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 28 }}>
          {SERVICES.slice(0, 3).map((s, i) => (
            <div key={i} style={{ opacity: v ? 1 : 0, transform: v ? "none" : "translateY(30px)", transition: `all .65s cubic-bezier(.16,1,.3,1) ${i * 0.08}s`, background: "#FAF8F5", border: "1px solid #E4E3DD", borderRadius: 12, overflow: "hidden" }}
            ...
            >
              ...
              <div style={{ padding: 28 }}>
                <h3 style={{ fontSize: 18, fontWeight: 700, color: "#14161C", marginBottom: 10 }}>{s.title}</h3>
...
function ProcessSection() {
  const [ref, v] = useInView();
  return (
    <section style={{ background: "#14161C", padding: "110px clamp(20px,5vw,80px)" }}>
      <div style={{ maxWidth: 1320, margin: "0 auto" }}>
    ...
    <section style={{ background: "#FAF8F5", padding: "110px clamp(20px,5vw,80px)" }}>
      <div style={{ maxWidth: 1320, margin: "0 auto" }}>
        <div ref={ref} style={{ textAlign: "center", marginBottom: 72, opacity: v ? 1 : 0, transform: v ? "none" : "translateY(20px)", transition: "all .7s cubic-bezier(.16,1,.3,1)" }}>
          <h2 style={{ fontSize: "clamp(30px,3vw,50px)", fontWeight: 800, color: "#14161C", letterSpacing: "-1px", marginBottom: 16 }}>
        ...
        <div className="cw-grid-3" style={{ gap: 24 }}>
          {cards.map((card, i) => (
            <div key={i} style={{ padding: "36px 32px", borderRadius: 16, border: "1px solid #E4E3DD", background: "#FAF8F5", opacity: v ? 1 : 0, transform: v ? "none" : "translateY(24px)", transition: `all .65s cubic-bezier(.16,1,.3,1) ${i * 0.07}s` }}
            ...
            >
              ...
              <h3 style={{ fontSize: 17, fontWeight: 700, color: "#14161C", marginBottom: 10 }}>{card.title}</h3>
...
function AnimatedTestimonials() {
  ...
  return (
    <section style={{ background: "#14161C", padding: "90px 0", overflow: "hidden" }}>
      <div ref={ref} style={{ textAlign: "center", marginBottom: 60, padding: "0 clamp(20px,5vw,80px)" }}>
        <h2 style={{ fontSize: "clamp(28px,3vw,48px)", fontWeight: 800, color: "#FAF8F5", letterSpacing: "-1px", opacity: v ? 1 : 0, transform: v ? "none" : "translateY(20px)", transition: "all .7s cubic-bezier(.16,1,.3,1) .1s" }}>What Our Clients Say.</h2>
      ...
      <div style={{ position: "relative" }}>
        <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: 120, background: "linear-gradient(to right, #14161C, transparent)", zIndex: 2, pointerEvents: "none" }} />
        <div style={{ position: "absolute", right: 0, top: 0, bottom: 0, width: 120, background: "linear-gradient(to left, #14161C, transparent)", zIndex: 2, pointerEvents: "none" }} />
...
function CTA() {
  const [ref, v] = useInView();
  return (
    <section style={{ background: "#14161C", padding: "110px clamp(20px,5vw,80px)", position: "relative", overflow: "hidden" }}>
      ...
      <div ref={ref} className="cw-grid-1auto" style={{ maxWidth: 1320, margin: "0 auto", gap: 80, alignItems: "center", position: "relative", zIndex: 1 }}>
        <div style={{ opacity: v ? 1 : 0, transform: v ? "none" : "translateY(20px)", transition: "all .8s cubic-bezier(.16,1,.3,1)" }}>
          <h2 style={{ fontSize: "clamp(36px,4.5vw,64px)", fontWeight: 800, color: "#FAF8F5", lineHeight: 1.06, letterSpacing: "-1.5px", marginBottom: 18 }}>Ready to build something extraordinary?</h2>
