      ...
      <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", justifyContent: "center", padding: "0 clamp(20px,5vw,80px)", paddingTop: 80 }}>
        <div style={{ maxWidth: 900 }}>
          <h1 style={{ fontSize: "clamp(52px,8vw,110px)", fontWeight: 800, color: "#fff", lineHeight: 0.95, letterSpacing: "-4px", marginBottom: 28, textTransform: "uppercase", opacity: v ? 1 : 0, transform: v ? "none" : "translateY(24px)", transition: "all .9s cubic-bezier(.16,1,.3,1) .4s" }}>
            WE BUILD<br />
            <span style={{ color: "#C9922F", fontStyle: "italic", textTransform: "none", letterSpacing: "-3px" }}>Brands</span> THAT<br />
            WIN ONLINE.
          </h1>
          ...
          <div style={{ display: "flex", gap: 16, flexWrap: "wrap", opacity: v ? 1 : 0, transform: v ? "none" : "translateY(12px)", transition: "all .7s cubic-bezier(.16,1,.3,1) .75s" }}>
            <Link to="/contact" style={{ display: "inline-flex", alignItems: "center", gap: 10, background: "#C9922F", color: "#fff", padding: "15px 32px", borderRadius: 50, textDecoration: "none", fontSize: 13, fontWeight: 700, letterSpacing: "0.05em", textTransform: "uppercase", transition: "all .25s", boxShadow: "0 8px 32px rgba(201,146,47,.5)" }}
              onMouseEnter={(e) => { e.currentTarget.style.background = "#d03d16"; e.currentTarget.style.transform = "translateY(-2px)"; }}
              onMouseLeave={(e) => { e.currentTarget.style.background = "#C9922F"; e.currentTarget.style.transform = "none"; }}
            >
              Start a Project
    ...
    <section style={{ background: "#fff", padding: "110px clamp(20px,5vw,80px)" }}>
      <div style={{ maxWidth: 1320, margin: "0 auto" }}>
        <div ref={ref} style={{ textAlign: "center", marginBottom: 64 }}>
          <h2 style={{ fontSize: "clamp(30px,3vw,50px)", fontWeight: 800, color: "#16161A", letterSpacing: "-1px", opacity: v ? 1 : 0, transform: v ? "none" : "translateY(20px)", transition: "all .7s cubic-bezier(.16,1,.3,1) .1s" }}>
          ...
          {SERVICES.slice(0, 3).map((s, i) => (
            ...
            >
              ...
              <div style={{ padding: 28 }}>
                <h3 style={{ fontSize: 18, fontWeight: 700, color: "#16161A", marginBottom: 10 }}>{s.title}</h3>
        ...
        <div style={{ textAlign: "center", marginTop: 52 }}>
          <Link to="/services" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "#16161A", color: "#fff", padding: "14px 32px", borderRadius: 8, textDecoration: "none", fontSize: 13, fontWeight: 700, transition: "all .25s" }}
...
function ProcessSection() {
  const [ref, v] = useInView();
  return (
    <section style={{ background: "#16161A", padding: "110px clamp(20px,5vw,80px)" }}>
      <div style={{ maxWidth: 1320, margin: "0 auto" }}>
        <div ref={ref} className="cw-grid-2" style={{ gap: "clamp(48px,6vw,100px)", alignItems: "center", marginBottom: 80 }}>
          <div style={{ opacity: v ? 1 : 0, transform: v ? "none" : "translateX(-20px)", transition: "all .8s cubic-bezier(.16,1,.3,1)" }}>
            <h2 style={{ fontSize: "clamp(30px,3vw,48px)", fontWeight: 800, color: "#fff", letterSpacing: "-1px", lineHeight: 1.1, marginBottom: 20 }}>
              How We Work.<br /><span style={{ color: "#C9922F" }}>Every Single Time.</span>
            </h2>
            ...
            <Link to="/about" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "#C9922F", color: "#fff", padding: "13px 28px", borderRadius: 8, textDecoration: "none", fontSize: 13, fontWeight: 700, transition: "all .25s" }}
              onMouseEnter={(e) => { e.currentTarget.style.background = "#d03d16"; e.currentTarget.style.transform = "translateY(-2px)"; }}
              onMouseLeave={(e) => { e.currentTarget.style.background = "#C9922F"; e.currentTarget.style.transform = "none"; }}
            >Our Process →</Link>
          </div>
          <div className="cw-grid-2" style={{ gap: 16, opacity: v ? 1 : 0, transform: v ? "none" : "translateX(20px)", transition: "all .8s cubic-bezier(.16,1,.3,1) .1s" }}>
            {PROCESS.slice(0, 4).map((p, i) => (
              <div key={i} style={{ background: "#1A1A1A", borderRadius: 12, padding: "24px 20px", border: "1px solid #1F2937", transition: "all .3s" }}
                onMouseEnter={(e) => { e.currentTarget.style.borderColor = "rgba(201,146,47,.4)"; e.currentTarget.style.transform = "translateY(-4px)"; }}
...
function WhyUs() {
  const [ref, v] = useInView();
  const cards = [
    { icon: "/icons/paint palette.png", bg: "#FFF0EB", color: "#C9922F", title: "Premium Design", desc: "Hand-crafted, pixel-perfect designs that convert visitors into paying customers every time." },
    ...
    <section style={{ background: "#fff", padding: "110px clamp(20px,5vw,80px)" }}>
      <div style={{ maxWidth: 1320, margin: "0 auto" }}>
        <div ref={ref} style={{ textAlign: "center", marginBottom: 72, opacity: v ? 1 : 0, transform: v ? "none" : "translateY(20px)", transition: "all .7s cubic-bezier(.16,1,.3,1)" }}>
          <h2 style={{ fontSize: "clamp(30px,3vw,50px)", fontWeight: 800, color: "#16161A", letterSpacing: "-1px", marginBottom: 16 }}>
        ...
        <div className="cw-grid-3" style={{ gap: 24 }}>
          {cards.map((card, i) => (
            ...
            >
              ...
              <h3 style={{ fontSize: 17, fontWeight: 700, color: "#16161A", marginBottom: 10 }}>{card.title}</h3>
              <p style={{ fontSize: 14, color: "#6B7280", lineHeight: 1.8 }}>{card.desc}</p>
            </div>
          ))}
        </div>
        <div style={{ textAlign: "center", marginTop: 52 }}>
          <Link to="/about" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "#16161A", color: "#fff", padding: "14px 28px", borderRadius: 8, textDecoration: "none", fontSize: 13, fontWeight: 700, transition: "all .25s" }}
            onMouseEnter={(e) => { e.currentTarget.style.background = "#C9922F"; e.currentTarget.style.transform = "translateY(-2px)"; }}
            onMouseLeave={(e) => { e.currentTarget.style.background = "#16161A"; e.currentTarget.style.transform = "none"; }}
          >Learn About Us →</Link>
        </div>
      ...
      <div style={{ maxWidth: 1320, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 24 }}>
          {WORKS.slice(0, 3).map((w, i) => (
            <div key={i} style={{ padding: 3, borderRadius: 14, background: i === 0 ? "linear-gradient(135deg, #C9922F, #ff9a00)" : i === 1 ? "linear-gradient(135deg, #C9922F, #cc0000)" : "linear-gradient(135deg, #C9922F, #ff6b6b)", opacity: v ? 1 : 0, transform: v ? "none" : "translateY(30px)", transition: `opacity .65s cubic-bezier(.16,1,.3,1) ${i * 0.08}s, transform .65s cubic-bezier(.16,1,.3,1) ${i * 0.08}s, box-shadow .3s`, boxShadow: "0 8px 32px rgba(201,146,47,.15)" }}
              onMouseEnter={(e) => { e.currentTarget.style.boxShadow = "0 20px 60px rgba(201,146,47,.45)"; e.currentTarget.querySelector("img").style.transform = "scale(1.08)"; e.currentTarget.querySelector(".overlay").style.opacity = "1"; e.currentTarget.querySelector(".overlay").style.transform = "translateY(0)"; }}
              onMouseLeave={(e) => { e.currentTarget.style.boxShadow = "0 8px 32px rgba(201,146,47,.15)"; e.currentTarget.querySelector("img").style.transform = "scale(1)"; e.currentTarget.querySelector(".overlay").style.opacity = "0"; e.currentTarget.querySelector(".overlay").style.transform = "translateY(20px)"; }}
            >
              <div style={{ position: "relative", height: 380, borderRadius: 11, overflow: "hidden", cursor: "pointer", background: "#e0e0e0" }}>
                ...
                <div className="overlay" style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,.82)", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", padding: 36, textAlign: "center", opacity: 0, transform: "translateY(20px)", transition: "all .4s cubic-bezier(.16,1,.3,1)" }}>
                  ...
                  <a href={w.url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, fontWeight: 700, color: "#fff", background: "#C9922F", padding: "10px 24px", borderRadius: 6, textDecoration: "none" }}>View Project ↗</a>
                </div>
              </div>
            </div>
          ))}
        ...
        <div style={{ textAlign: "center", marginTop: 52 }}>
          <Link to="/work" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "#16161A", color: "#fff", padding: "14px 32px", borderRadius: 8, textDecoration: "none", fontSize: 13, fontWeight: 700, transition: "all .25s" }}
            onMouseEnter={(e) => { e.currentTarget.style.background = "#C9922F"; e.currentTarget.style.transform = "translateY(-2px)"; }}
            onMouseLeave={(e) => { e.currentTarget.style.background = "#16161A"; e.currentTarget.style.transform = "none"; }}
...
function AnimatedTestimonials() {
  ...
  return (
    <section style={{ background: "#16161A", padding: "90px 0", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative" }}>
        <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: 120, background: "linear-gradient(to right, #16161A, transparent)", zIndex: 2, pointerEvents: "none" }} />
        <div style={{ position: "absolute", right: 0, top: 0, bottom: 0, width: 120, background: "linear-gradient(to left, #16161A, transparent)", zIndex: 2, pointerEvents: "none" }} />
        <div style={{ display: "flex", gap: 24, width: "max-content", animation: "testimonialScroll 40s linear infinite" }}>
          {doubled.map((t, i) => (
            <div key={i} style={{ width: 360, flexShrink: 0, background: "#1A1A1A", border: "1px solid rgba(255,255,255,.08)", borderRadius: 16, padding: "32px 28px", transition: "border-color .3s" }}
              onMouseEnter={(e) => { e.currentTarget.style.borderColor = "rgba(201,146,47,.5)"; e.currentTarget.parentElement.style.animationPlayState = "paused"; }}
            ...
            >
              <div style={{ color: "#C9922F", fontSize: 14, letterSpacing: 3, marginBottom: 18 }}>★★★★★</div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                <div style={{ width: 44, height: 44, borderRadius: "50%", overflow: "hidden", flexShrink: 0 }}>
                  {t.photo ? <img src={t.photo} alt={t.author} style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <div style={{ width: "100%", height: "100%", background: "linear-gradient(135deg, #C9922F, #E0B563)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "#fff" }}>{t.initials}</div>}
                </div>
                <div>
...
function CTA() {
  const [ref, v] = useInView();
  return (
    <section style={{ background: "#C9922F", padding: "110px clamp(20px,5vw,80px)", position: "relative", overflow: "hidden" }}>
