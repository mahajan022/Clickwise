...
const SERVICE_CARDS = [
  { label: "Ad Shoots", sub: "Photography & Video", icon: "ti-camera", color: "#FBF3E4", accent: "#C9922F", span: 2, img: "/adShoot.png" },
  { label: "Social Media Management", sub: "Content & Community", icon: "ti-brand-instagram", color: "#F7F7F5", accent: "#16161A", span: 1, img: "/socialMedia.png" },
  { label: "Web Development", sub: "React & Custom Code", icon: "ti-code", color: "#EEF2FF", accent: "#4F46E5", span: 1, img: "/webDev.png" },
  { label: "Meta Ads", sub: "Paid Social & Campaigns", icon: "ti-brand-meta", color: "#FBF3E4", accent: "#C9922F", span: 1, img: "/metaAds.png" },
  { label: "Content Strategy", sub: "Planning & Creation", icon: "ti-pencil", color: "#F7F7F5", accent: "#16161A", span: 1, img: "/growthStrategy.png" },
  { label: "Growth Dashboards", sub: "Analytics & Reporting", icon: "ti-chart-line", color: "#F0FBF4", accent: "#16A34A", span: 2, img: "/growth.png" },
  { label: "Web Redesign", sub: "Conversion Focused", icon: "ti-layout", color: "#F7F7F5", accent: "#16161A", span: 1, img: "/webRede.png" },
  { label: "Brand Identity", sub: "Logo & Visual System", icon: "ti-vector-triangle", color: "#FBF3E4", accent: "#C9922F", span: 1, img: "/brandIdentity.png" },
  { label: "Google Ads", sub: "Search & Display", icon: "ti-brand-google", color: "#FFFBF0", accent: "#D97706", span: 1, img: "/googleAds.png" },
  { label: "Reels & Video Ads", sub: "Short-form Production", icon: "ti-player-play", color: "#F7F7F5", accent: "#16161A", span: 1, img: "/reels.png" },
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: span === 2 ? "16px 28px 26px" : "14px 22px 20px" }}>
        ...
        <h3 style={{ fontSize: span === 2 ? 22 : 15, fontWeight: 700, color: "#16161A", lineHeight: 1.25, margin: 0 }}>{card.label}</h3>
      </div>
    </div>
  );
}
    ...
    <section ref={sectionRef} style={{ background: "#F7F7F5", padding: "120px clamp(20px,5vw,80px)", overflow: "hidden" }}>
      <div style={{ maxWidth: 1320, margin: "0 auto" }}>
        <div ref={ref2} style={{ marginBottom: 64, opacity: v2 ? 1 : 0, transform: v2 ? "none" : "translateY(20px)", transition: "all .8s" }}>
          <h2 style={{ fontSize: "clamp(30px,3.5vw,54px)", fontWeight: 800, color: "#16161A", letterSpacing: "-1px", lineHeight: 1.1, maxWidth: 680 }}>
...
function ProcessHero({ ref4, v4 }) {
  return (
    <section style={{ position: "relative", width: "100%", minHeight: "70vh", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      >
        <h2 style={{ fontSize: "clamp(40px,5.5vw,80px)", fontWeight: 900, color: "#16161A", letterSpacing: "-2.5px", lineHeight: 1.02, marginBottom: 28 }}>
...
function WhyClickwise({ ref6, v6 }) {
  return (
    <section style={{ background: "#16161A", padding: "120px clamp(20px,5vw,80px)" }}>
        ...
        <div className="cw-grid-2" style={{ gap: 18, opacity: v6 ? 1 : 0, transform: v6 ? "none" : "translateX(32px)", transition: "all 1s cubic-bezier(.16,1,.3,1) 0.15s" }}>
          {VALUES.map((val, i) => (
            <div key={i}
              ...
              onMouseEnter={(e) => { e.currentTarget.style.borderColor = "rgba(201,146,47,0.5)"; e.currentTarget.style.transform = "translateY(-4px)"; }}
  ...
  return (
    ...
    >
      <div style={{
        ...
        background: "#C9922F",
        boxShadow: v ? "0 0 0 5px rgba(201,146,47,0.18)" : "0 0 0 3px rgba(201,146,47,0.10)",
        transform: v ? "scale(1)" : "scale(.6)",
        transition: "all .5s cubic-bezier(.16,1,.3,1)",
      }} />
      <div style={{ fontSize: 11, fontWeight: 800, color: "#C9922F", letterSpacing: "0.1em", marginBottom: 10 }}>{m.year}</div>
      <h3 style={{ fontSize: 18, fontWeight: 700, color: "#16161A", marginBottom: 10 }}>{m.title}</h3>
...
export default function About() {
  ...
  <meta name="description" content="Clicks&Ads is a full-service digital agency in Mumbai. Meet the web development company and marketing agency behind 50+ brand projects." />
      ...
      <section style={{ background: "#fff", padding: "120px clamp(20px,5vw,80px)" }}>
        <div className="cw-grid-2" style={{ maxWidth: 1320, margin: "0 auto", gap: "clamp(60px,7vw,120px)", alignItems: "center" }}>
          ...
          <div ref={ref1} style={{ opacity: v1 ? 1 : 0, transform: v1 ? "none" : "translateX(-32px)", transition: "all 1s cubic-bezier(.16,1,.3,1)" }}>
            <h2 style={{ fontSize: "clamp(28px,3vw,48px)", fontWeight: 800, color: "#16161A", letterSpacing: "-1px", lineHeight: 1.15, marginBottom: 28 }}>
          ...
          <div ref={ref1} style={{ opacity: v1 ? 1 : 0, transform: v1 ? "none" : "translateX(32px)", transition: "all 1s cubic-bezier(.16,1,.3,1) 0.15s", position: "relative" }}>
            ...
            <div style={{ position: "absolute", top: -16, right: -16, background: "#C9922F", borderRadius: 14, padding: "18px 22px", boxShadow: "0 16px 40px rgba(201,146,47,0.35)", animation: "floatA 4s ease-in-out infinite" }}>
            ...
            <div style={{ position: "absolute", bottom: 28, left: -20, background: "#fff", border: "1px solid #E4E3DD", borderRadius: 12, padding: "14px 18px", boxShadow: "0 12px 32px rgba(17,17,17,0.12)", animation: "floatB 5s ease-in-out infinite" }}>
              <div style={{ fontSize: 20, fontWeight: 900, color: "#C9922F", letterSpacing: "-0.5px", lineHeight: 1 }}>100%</div>
        ...
        <div className="cw-grid-4" style={{ maxWidth: 1320, margin: "72px auto 0", gap: 32, paddingTop: 56, borderTop: "1px solid #E4E3DD" }}>
          {STATS.map((s, i) => (
            <div key={i} style={{ opacity: v1 ? 1 : 0, transform: v1 ? "none" : "translateY(20px)", transition: `all .8s cubic-bezier(.16,1,.3,1) ${i * 0.1 + 0.3}s` }}>
              <div style={{ fontSize: 36, fontWeight: 900, color: "#C9922F", letterSpacing: "-1px", lineHeight: 1 }}>{s.v}</div>
      ...
      <section style={{ background: "#fff", padding: "120px clamp(20px,5vw,80px)" }}>
        <div style={{ maxWidth: 1320, margin: "0 auto" }}>
          <div ref={ref3} style={{ marginBottom: 80, opacity: v3 ? 1 : 0, transform: v3 ? "none" : "translateY(20px)", transition: "all .8s" }}>
            <h2 style={{ fontSize: "clamp(30px,3.5vw,54px)", fontWeight: 800, color: "#16161A", letterSpacing: "-1px", lineHeight: 1.1 }}>Four years. No shortcuts.</h2>
          </div>
          <div className="cw-grid-2" style={{ gap: "clamp(60px,7vw,120px)", alignItems: "start" }}>
            <div style={{ position: "relative", paddingLeft: 28 }}>
              <div style={{ position: "absolute", left: 0, top: 8, bottom: 0, width: 1, background: "linear-gradient(to bottom, #C9922F 0%, #E4E3DD 100%)" }} />
              {milestones.map((m, i) => (
      ...
      {/* ══ STATS RED BAND ══ */}
      <section style={{ background: "#C9922F", padding: "90px clamp(20px,5vw,80px)" }}>
      ...
      <section style={{ background: "#fff", padding: "120px clamp(20px,5vw,80px)" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div ref={ref5} style={{ textAlign: "center", marginBottom: 80, opacity: v5 ? 1 : 0, transition: "all .7s" }}>
            <h2 style={{ fontSize: "clamp(30px,3.5vw,52px)", fontWeight: 800, color: "#16161A", letterSpacing: "-1px" }}>Frequently Asked Questions</h2>
          </div>
          <div className="cw-grid-2" style={{ gap: 32, alignItems: "start" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {faqs.map((faq, i) => (
                <div key={i} onClick={() => setActiveFAQ(i)}
                  style={{ padding: "18px 24px", borderRadius: 10, cursor: "pointer", background: activeFAQ === i ? "#C9922F" : "#F7F7F5", border: `1px solid ${activeFAQ === i ? "#C9922F" : "#E4E3DD"}`, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, transition: "all .3s" }}
                  onMouseEnter={(e) => { if (activeFAQ !== i) e.currentTarget.style.borderColor = "rgba(201,146,47,0.4)"; }}
                  onMouseLeave={(e) => { if (activeFAQ !== i) e.currentTarget.style.borderColor = "#E4E3DD"; }}
                >
                  <span style={{ fontSize: 14, fontWeight: 600, color: activeFAQ === i ? "#fff" : "#16161A" }}>{faq.q}</span>
                  <span style={{ color: activeFAQ === i ? "#fff" : "#C9922F", fontSize: 18, flexShrink: 0 }}>{activeFAQ === i ? "✕" : "+"}</span>
                </div>
              ))}
            </div>
            <div className="cw-unsticky-mobile" style={{ background: "#F7F7F5", borderRadius: 16, padding: "40px 36px", border: "1px solid #E4E3DD", position: "sticky", top: 100 }}>
              <div style={{ width: 40, height: 3, background: "#C9922F", borderRadius: 2, marginBottom: 24 }} />
              <h3 style={{ fontSize: 18, fontWeight: 700, color: "#16161A", marginBottom: 16, lineHeight: 1.4 }}>{faqs[activeFAQ].q}</h3>
      ...
      {/* ══ CTA ══ */}
      <section style={{ background: "#C9922F", padding: "120px clamp(20px,5vw,80px)" }}>
